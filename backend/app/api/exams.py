...
class TemplateZoneRevisionResponse(BaseModel):
    revision_number: int
    change_type: str
    change_reason: str | None
    changed_by: str | None
    changed_at: datetime
    zone: TemplateZonePreview


class TemplateZoneHistoryResponse(BaseModel):
    zone_id: str
    revisions: list[TemplateZoneRevisionResponse]


class TemplateZoneRollbackResponse(BaseModel):
    zone_id: str
    applied_revision_number: int
    zone: TemplateZonePreview
...
async def _create_zone_revision(
    ...
    last_revision_result = await db.execute(
        select(TemplateZoneRevision.revision_number)
        .where(TemplateZoneRevision.zone_id == zone.id)
        .order_by(TemplateZoneRevision.revision_number.desc())
        .limit(1)
    )
    last_revision_number = last_revision_result.scalar_one_or_none()
    next_revision = (last_revision_number or 0) + 1
...
async def extract_template_zones(
    ...
    if request.overwrite_existing:
        ...
        existing_zones_result = await db.execute(
        ...
        for existing_zone in existing_zones:
            await _create_zone_revision(
                ...
                existing_zone,
                change_type="DELETE",
                change_reason="overwrite_existing_extraction",
            )
    ...
    for zone in extracted_zones:
        ...
        await _create_zone_revision(
            ...
            change_reason="automatic_template_extraction",
        )
...
async def update_template_zone(
    ...
    await _create_zone_revision(
        ...
        change_reason="zone_patch_update",
    )
...
async def replace_template_zones(
    ...
    previous_zones_result = await db.execute(
        select(TemplateZone).where(TemplateZone.template_id == template.id)
    )
    previous_zones = previous_zones_result.scalars().all()
    ...
    for zone in request.zones:
        ...
        await _create_zone_revision(
            ...
            change_type="MANUAL_EDIT",
            changed_by=current_user.id,
            change_reason="bulk_replace_insert",
        )
...
async def validate_template_zones(
    ...
    for zone in zones:
        ...
        await _create_zone_revision(
            ...
            changed_by=current_user.id,
            change_reason="template_validate",
        )
...
@router.get(
    "/template-zones/{zone_id}/history",
    response_model=TemplateZoneHistoryResponse,
)
async def get_template_zone_history(
    zone_id: UUID,
    workspace_id: UUID = Depends(get_workspace_id),
    membership: WorkspaceMember = Depends(require_teacher),
    db: AsyncSession = Depends(get_db),
):
    """Get revision history for a template zone."""
    zone_result = await db.execute(
        select(TemplateZone)
        .join(ExamTemplate, ExamTemplate.id == TemplateZone.template_id)
        .join(ExamVersion, ExamVersion.id == ExamTemplate.exam_version_id)
    ...
    revisions_result = await db.execute(
        ...
        .where(TemplateZoneRevision.zone_id == zone.id)
        .order_by(TemplateZoneRevision.revision_number.asc())
    )
    revisions = revisions_result.scalars().all()

    return TemplateZoneHistoryResponse(
        zone_id=str(zone.id),
        revisions=[
            TemplateZoneRevisionResponse(
                revision_number=rev.revision_number,
                change_type=rev.change_type,
                change_reason=rev.change_reason,
                changed_by=str(rev.changed_by) if rev.changed_by else None,
                changed_at=rev.changed_at,
                zone=TemplateZonePreview(
                    id=str(zone.id),
                    page_index=rev.page_index,
                    question_key=rev.question_key,
                    bbox=TemplateZoneBBox(
                        x=rev.bbox_x,
                        y=rev.bbox_y,
                        width=rev.bbox_width,
                        height=rev.bbox_height,
                    ),
                    pad_ratio=rev.pad_ratio,
...
@router.post(
    "/template-zones/{zone_id}/rollback/{revision_number}",
    response_model=TemplateZoneRollbackResponse,
)
async def rollback_template_zone_revision(
    zone_id: UUID,
    revision_number: int,
    workspace_id: UUID = Depends(get_workspace_id),
    membership: WorkspaceMember = Depends(require_teacher),
    ...
    """Rollback a zone to a selected revision snapshot."""
    zone_result = await db.execute(
        select(TemplateZone)
        .join(ExamTemplate, ExamTemplate.id == TemplateZone.template_id)
        .join(ExamVersion, ExamVersion.id == ExamTemplate.exam_version_id)
    ...
    target_result = await db.execute(
        select(TemplateZoneRevision)
        .where(TemplateZoneRevision.zone_id == zone.id)
        .where(TemplateZoneRevision.revision_number == revision_number)
    )
    target_revision = target_result.scalar_one_or_none()
    if target_revision is None:
        raise HTTPException(status_code=404, detail="Revision not found")

    zone.page_index = target_revision.page_index
    zone.question_key = target_revision.question_key
    zone.bbox_x = target_revision.bbox_x
    zone.bbox_y = target_revision.bbox_y
    zone.bbox_width = target_revision.bbox_width
    zone.bbox_height = target_revision.bbox_height
    zone.pad_ratio = target_revision.pad_ratio
    zone.confidence = target_revision.confidence
    zone.source = "manual"
    zone.edit_source = "manual"
    zone.is_validated = target_revision.is_validated
    zone.last_edited_by = current_user.id
    zone.last_edited_at = datetime.utcnow()
    if zone.is_validated:
        zone.validated_by = current_user.id
    ...
    await _create_zone_revision(
        ...
        changed_by=current_user.id,
        change_reason=f"rollback_to_{revision_number}",
    )

    await db.commit()
    await db.refresh(zone)

    return TemplateZoneRollbackResponse(
        zone_id=str(zone.id),
        applied_revision_number=revision_number,
